...
describe("CachedModelsTypeRepository", () => {
	...
	beforeEach(async () => {
		...
		repository = new CachedModelsTypeRepository(innerRepository, cacheInstance);
	});
	...
	describe("findBySlug", () => {
		...
		it("should invalidate stale slug cache when slug changed", async () => {
			...
			await cacheInstance.set(slugKey("old-slug"), modelsType.id, "EX", 3600);
	...
	describe("delete", () => {
		...
		it("should remove cached entries after delete", async () => {
			...
			const cachedBySlug = await cacheInstance.get(slugKey(modelsType.slug));
