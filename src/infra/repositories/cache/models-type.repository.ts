...
export class ModelsTypeRepository implements IModelsTypeRepository {
    ...
    async findMany(page: number): Promise<IModelsType[]> {
        ...
        await Promise.all([
            ...
            this.cache.set(
                key,
                JSON.stringify(
                    targetModelsTypes.map((modelsType) => modelsType.id),
                ),
                "EX",
    ...
    private async findManyByIds(ids: string[]): Promise<IModelsType[]> {
        ...
        return ids
            ...
            .filter(
                (modelsType): modelsType is IModelsType =>
    ...
    async update(modelsType: IModelsType): Promise<void> {
        ...
        if (_cachedModelsType) {
            ...
            await Promise.all([
                this.cache.del(
                    `${ModelsType[EntitySource]}::$${cachedModelsType.id}`,
                ),
                this.cache.del(
                    `${ModelsType[EntitySource]}::${cachedModelsType.slug}`,
                ),
            ]);
        }
