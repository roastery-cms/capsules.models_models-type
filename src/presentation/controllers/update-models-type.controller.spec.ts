...
describe("UpdateModelsTypeController", () => {
    ...
    it("should update a models type and return 200", async () => {
        ...
        const { status } = await api["models-types"][modelsType.id].patch(
            { name: "Updated Name" },
            options,
        );
    ...
    it("should return the updated models type payload", async () => {
        ...
        const { data } = await api["models-types"][modelsType.id].patch(
            { name: newName },
            options,
        );
    ...
    it("should return error for a non-existent id", async () => {
        ...
        const { status } = await api["models-types"][generateUUID()].patch(
            { name: "Updated" },
            options,
        );
